...
class CarDashboard(ctk.CTk):
    def __init__(self):
        ...
        self.val_flow = 0.0
        self.pulse_count = 0
        self.pulse_history = [0, 0, 0]  # 3-second memory buffer for smoothing
    ...
    def close_app(self):
        self.stop_logging()
        GPIO.cleanup() # Critical: Free the hardware pins before exit
        self.destroy()
    ...
    def logging_loop(self):
        ...
        while self.running:
            ...
            if self.writer and current_time != last_logged_time:
                ...
                # Slide the history window and add the new pulse count
                self.pulse_history.pop(0)
                self.pulse_history.append(self.pulse_count)
                
                # Reset pulse counter instantly for the next second
                self.pulse_count = 0 
                
                # Calculate the average pulses over the last 3 seconds
                avg_pulses = sum(self.pulse_history) / len(self.pulse_history)

                # Apply the newly calibrated 0.33 constant
                self.val_flow = round(avg_pulses / 0.42, 2)
